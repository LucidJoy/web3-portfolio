...
const Hero = ({ state }) => {
  ...
  const [imageLinkCid, setImageLinkCid] = useState("");

  useEffect(() => {
    const desc = async () => {
      const descText = await contract.methods.description().call();
      const image = await contract.methods.imageLink().call();
      setDescription(descText);
      setImageLinkCid(image);
    };
    contract && desc();
  }, [state]);
      ...
      <div className='container'>
        ...
        <div className='hero-img'>
          <div className='img-container'>
            <img
              src={`https://jade-fundamental-hawk-687.mypinata.cloud/ipfs/${imageLinkCid}`}
              alt='profilePhoto'
            />
          </div>
        </div>
